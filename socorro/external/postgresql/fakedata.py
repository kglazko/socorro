...
class RawCrashes(BaseTable):
    ...
    def generate_rows(self):
        ...
        for crashid, date_processed, in crash_ids:
            android_device = secrets.SystemRandom().choice(android)
            raw_crash = {
                "uuid": crashid,
                "IsGarbageCollecting": "1",
                "AdapterVendorID": secrets.SystemRandom().choice(vendors),
                "AdapterDeviceID": secrets.SystemRandom().choice(devices),
